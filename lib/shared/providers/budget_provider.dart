...
class CategoryBudgetProgress {
  ...
  double get percentage => budgetedAmount > 0 ? (spentAmount / budgetedAmount).clamp(0.0, 1.0) : 0.0;
...
final budgetProgressProvider = Provider<AsyncValue<List<CategoryBudgetProgress>>>((ref) {
  final categoriesState = ref.watch(categoryProvider);
  final budgetsState = ref.watch(budgetProvider);
  final transactionsState = ref.watch(transactionProvider);

  if (categoriesState is AsyncLoading || budgetsState is AsyncLoading || transactionsState is AsyncLoading) {
    return const AsyncValue.loading();
  }

  if (categoriesState is AsyncError) return AsyncValue.error(categoriesState.error!, categoriesState.stackTrace!);
  if (budgetsState is AsyncError) return AsyncValue.error(budgetsState.error!, budgetsState.stackTrace!);
  if (transactionsState is AsyncError) return AsyncValue.error(transactionsState.error!, transactionsState.stackTrace!);
  ...
  // Build progress list
  final expenseCategories = categories.where((c) => c.type == CategoryType.expense || c.type == CategoryType.both).toList();
  
  final List<CategoryBudgetProgress> progressList = expenseCategories.map((cat) {
